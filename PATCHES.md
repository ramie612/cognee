...
| `fix(datasets): authorize /raw by dataset permission, not by who authored the item` | bugfix | `GET /datasets/{ds}/data/{id}/raw` verified READ permission on the dataset and confirmed the item belonged to it, then discarded both and called `get_data(user.id, data_id)`, which re-scopes to the caller as the data record's OWNER and raises `UnauthorizedDataAccessError` otherwise. In a SHARED dataset only an item's author could read it back: every teammate, and even the dataset's own owner, got 401 on someone else's item, which made `cognee_memories` show teammates an empty list. It protected nothing, since `/recall` already returns the same content to anyone holding dataset read. Uses the already-authorized record instead. | `api/v1/datasets/routers/get_datasets_router.py` |
